...
export function criarCardVisitaHTML({ nome, idade, cidade, profissao, email, telefone, fotoUrl, instagram, linkedin }) {
  // Limpa caracteres especiais do telefone para criar o link do WhatsApp
  const telNumeros = telefone.replace(/\D/g, "");
  const zapLink = telNumeros.length >= 10 ? `https://wa.me/55${telNumeros}` : `tel:${telNumeros}`;

  // Monta os botões com ícones para redes sociais (Instagram e LinkedIn)
  let instaLink = "";
  if (instagram) {
  ...
  // Bloco de redes sociais que fica na base da 2ª coluna
  const socialBlock = (instaLink || linkedinLink) ? `
  ...
  // Estrutura com nome no topo e os dados divididos em Coluna 1 (Pessoal) e Coluna 2 (Contato/Social)
  return `
    <div class="card-visita">
      ...
      <div class="card-info">
        ...
        <div class="card-corpo-colunas">
          <!-- Coluna 1: Idade, Cidade e Profissão/Serviço -->
          ...
          <!-- Coluna 2: E-mail, Telefone/Zap e Redes Sociais -->
          <ul class="card-coluna">
            <li>
              <i class="bi bi-envelope-fill"></i>
              <a href="mailto:${email}" class="texto-cortado" title="${email}">${email}</a>
            </li>
            <li>
